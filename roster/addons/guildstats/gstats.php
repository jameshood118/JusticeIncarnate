...
$result_menu = $wowdb->query($query) or die_quietly($wowdb->error(), 'Database Error', basename(__FILE__), __LINE__, $query);
$classRow = $wowdb->getrow($result_menu);
$content .= addTopTableLine(localize('classhiaverage'), $classRow['class'], $classRow['classaverage'], true, ($ShowGuildBars == 1)) . "
	" ;
...
$result_menu = $wowdb->query($query) or die_quietly($wowdb->error(), 'Database Error', basename(__FILE__), __LINE__, $query);
$classRow = $wowdb->getrow($result_menu);
$content .= addTopTableLine(localize('classloaverage'), $classRow['class'], $classRow['classaverage'], true, ($ShowGuildBars == 1)) . "
	" ;

// Classes with the most and least number of members at level 70
$query = "SELECT `class`, count(`member_id`) as `classcount` FROM `" . ROSTER_MEMBERSTABLE . "` " . $whereclause . " AND `level` = 70 GROUP BY `class` ORDER BY `classcount` DESC LIMIT 1";
$result_menu = $wowdb->query($query) or die_quietly($wowdb->error(), 'Database Error', basename(__FILE__), __LINE__, $query);
$classRow = $wowdb->getrow($result_menu);
$content .= addTopTableLine(localize('classmost70'), $classRow['class'], $classRow['classcount'], false, false) . "
	" ;
$query = "SELECT `class`, count(`member_id`) as `classcount` FROM `" . ROSTER_MEMBERSTABLE . "` " . $whereclause . " AND `level` = 70 GROUP BY `class` ORDER BY `classcount` LIMIT 1";
$result_menu = $wowdb->query($query) or die_quietly($wowdb->error(), 'Database Error', basename(__FILE__), __LINE__, $query);
$classRow = $wowdb->getrow($result_menu);
$content .= addTopTableLine(localize('classleast70'), $classRow['class'], $classRow['classcount'], false, false) . "
	" ;

// Classes with the most and least number of members between level 60 and 69
$query = "SELECT `class`, count(`member_id`) as `classcount` FROM `" . ROSTER_MEMBERSTABLE . "` " . $whereclause . " AND `level` > 59 AND `level` < 70 GROUP BY `class` ORDER BY `classcount` DESC LIMIT 1";
$result_menu = $wowdb->query($query) or die_quietly($wowdb->error(), 'Database Error', basename(__FILE__), __LINE__, $query);
$classRow = $wowdb->getrow($result_menu);
$content .= addTopTableLine(localize('classmost60'), $classRow['class'], $classRow['classcount'], false, false) . "
	" ;
$query = "SELECT `class`, count(`member_id`) as `classcount` FROM `" . ROSTER_MEMBERSTABLE . "` " . $whereclause . " AND `level` > 59 AND `level` < 70 GROUP BY `class` ORDER BY `classcount` LIMIT 1";
$result_menu = $wowdb->query($query) or die_quietly($wowdb->error(), 'Database Error', basename(__FILE__), __LINE__, $query);
$classRow = $wowdb->getrow($result_menu);
...
$levels = " AND `" . ROSTER_MEMBERSTABLE . "`.`level` = 70";
...
$num_guild_lvl_70 = $wowdb->num_rows($result_menu);
$levels = " AND `" . ROSTER_MEMBERSTABLE . "`.`level` > 59 and `" . ROSTER_MEMBERSTABLE . "`.`level` < 70";
$result_menu = $wowdb->query($MemberSelect . $whereclause . $levels) or die_quietly($wowdb->error(), 'Database Error', basename(__FILE__), __LINE__, $MemberSelect . $whereclause . $levels);
$num_guild_lvl_60_69 = $wowdb->num_rows($result_menu);
...
while ($classRow = $wowdb->getrow($classresult))
{
	...
	$num_lvl_70 = $wowdb->num_rows($result_menu);
	...
	$num_lvl_70_alts = $wowdb->num_rows($result_menu);
	...
	$num_lvl_70_loaded = $wowdb->num_rows($result_menu);
	
	$levels = " AND `" . ROSTER_MEMBERSTABLE . "`.`level` > 59 and `" . ROSTER_MEMBERSTABLE . "`.`level` < 70";
	$result_menu = $wowdb->query($select11 . $levels) or die_quietly($wowdb->error(), 'Database Error', basename(__FILE__), __LINE__, $select11 . $levels);
	$num_lvl_60_69 = $wowdb->num_rows($result_menu);
	$result_menu = $wowdb->query($select21 . $levels) or die_quietly($wowdb->error(), 'Database Error', basename(__FILE__), __LINE__, $select21 . $levels);
	$num_lvl_60_69_alts = $wowdb->num_rows($result_menu);
	$result_menu = $wowdb->query($select31 . $levels) or die_quietly($wowdb->error(), 'Database Error', basename(__FILE__), __LINE__, $select31 . $levels);
	$num_lvl_60_69_loaded = $wowdb->num_rows($result_menu);
	...
	$content .= "<table width=\"100%\" cellpadding=\"2\" cellspacing=\"2\">
		...
		" . addTableLine(localize('level') . " 70", $num_lvl_70, $num_lvl_70_alts, $num_lvl_70_loaded, false, $num_guild_lvl_70, false) . "
		" . addTableLine(localize('level') . " 60-69", $num_lvl_60_69, $num_lvl_60_69_alts, $num_lvl_60_69_loaded, false, $num_guild_lvl_60_69, false) . "
...
global $num_guild, $num_non_alts, $num_alts, $leveldecimal, $roster_conf;
	if ($bBar)
	{
		$mainPercent = round(($Main / 70) * 100);
		$altPercent = round(($Alt / 70) * 100);
		$uploadedPercent = round(($Uploaded / 70) * 100);
		return "		<tr>
			<td>
				" . $Level . "
			</td>
			<td>
				<div style=\"cursor:default;\">
					...
					<table class=\"expOutline\" border=0 cellpadding=0 cellspacing=0 width=70>
						<tr>
							<td style=\"background-image: url('" . $roster_conf['interface_url'] . "expbar-var2.gif');\" width=\"" . $mainPercent . "%\"><img src=\"" . $roster_conf['interface_url'] . "pixel.gif\" height=\"14\" width=\"1\" alt=\"\"></td>
			...
			<td>
				<div style=\"cursor:default;\">
					...
					<table class=\"expOutline\" border=0 cellpadding=0 cellspacing=0 width=70>
						<tr>
							<td style=\"background-image: url('" . $roster_conf['interface_url'] . "expbar-var2.gif');\" width=\"" . $altPercent . "%\"><img src=\"" . $roster_conf['interface_url'] . "pixel.gif\" height=\"14\" width=\"1\" alt=\"\"></td>
			...
			<td>
				<div style=\"cursor:default;\">
					...
					<table class=\"expOutline\" border=0 cellpadding=0 cellspacing=0 width=70>
						<tr>
							<td style=\"background-image: url('" . $roster_conf['interface_url'] . "expbar-var2.gif');\" width=\"" . $uploadedPercent . "%\"><img src=\"" . $roster_conf['interface_url'] . "pixel.gif\" height=\"14\" width=\"1\" alt=\"\"></td>
...
function addTopTableLine($text, $class, $num, $bround, $bBar)
{
	...
	if ($bBar)
	{
		$mainPercent = round(($count / 70) * 100);
		return "		<tr>
			<td>
				" . $text . "
			</td>
